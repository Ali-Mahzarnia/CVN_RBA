...
for (i in 1:length(file_list)) {
  ...
  len=length(temp$ad_mean)
  # temp$ad_mean
  #print(sum(temp$ad_mean))
  # print( sum(temp$ad_mean[2:len]))
  #if (sum(temp$ad_mean[2:len])==0) {volumes[i,2:len] = 0}
  #else 
  ...
  temp[missings,]$ad_mean =0 
  temp[missings,]$structure = Atlas$Structure[Atlas$index%in%missings] 
  
  }
  
    volumes[i,2:333]=temp$ad_mean[1:332]#/ sum(temp$ad_mean[1:332])
  # print(sum(as.numeric(volumes[i,2:len])))
  # whole_volume[i,2]=sum(temp$ad_mean[-noreadcsf])
...
write.xlsx2(table, "table_ad.xlsx", sheetName = "Significant" )
...
lastplot=annotate_figure(plot, top = text_grob("ad_9_violin", 
                                               color = "black", face = "bold", size = 14))
mywidth<-8
myheight<-4

ggsave(paste0('ad_9_violins.pdf'), plot = lastplot, device='pdf', scale=1, width=5*mywidth, height=5*myheight, unit=c("in"), dpi=200)
...
ggsave(paste0('ad_9_violins_opposite.pdf'), plot = lastplot, device='pdf', scale=1, width=5*mywidth, height=5*myheight, unit=c("in"), dpi=200)
...
write.xlsx2(table, "table_ad.xlsx", sheetName = "All", append=TRUE )
