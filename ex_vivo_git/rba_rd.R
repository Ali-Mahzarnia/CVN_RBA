...
len=length(temp$rd_mean)
volumes=matrix(NA,length(file_list),333)
noreadcsf=c(148,152,161,314,318,327)



Atlas_path = '/Users/ali/Desktop/Aug23/CVN/rba/exvivo/CHASSSYMM3AtlasLegends.xlsx'
...
for (i in 1:length(file_list)) {
  ...
  len=length(temp$rd_mean)
  # temp$rd_mean
  #print(sum(temp$rd_mean))
  # print( sum(temp$rd_mean[2:len]))
  #if (sum(temp$rd_mean[2:len])==0) {volumes[i,2:len] = 0}
  #else 
  ...
  temp[missings,]$rd_mean =0 
  temp[missings,]$structure = Atlas$Structure[Atlas$index%in%missings] 
  
  }
  
    volumes[i,2:333]=temp$rd_mean[1:332] #/ sum(temp$rd_mean[1:332])
  # print(sum(as.numeric(volumes[i,2:len])))
  # whole_volume[i,2]=sum(temp$rd_mean[-noreadcsf])
...
path_master='/Users/ali/Desktop/Aug23/CVN/rba/exvivo/MasterSheet_Experiments2021.xlsx'
...
#datatemp=datatemp[datatemp$Treatement=="APOE22",]

len = sum(datatemp$DWI%in%volumes$V1)
...
for (j in 1:dim(temp_bind)[1]) {
  ...
  index_master = which(volumes[,1] == datatemp$DWI[j])
...
#   lm <- lm( as.numeric(var2) ~Age_Months*as.factor(Sex)*as.factor(Treatement)* as.factor(Treatment),data=temp_bind )
#   
#   # pvals=an$`Pr(>F)`
#   temp=sig_result[,i]
#   eff=eta_squared(lm, partial = TRUE)
...
write.xlsx2(table, "table_rd.xlsx", sheetName = "Significant" )




table = as.data.frame(table)
table$diff= as.numeric(table$`Mean (Se)`)-as.numeric( table$`Mean (Tr)`)
...
for (j in 1:length(name_of_survivors_9)) {
  ...
  plot_list[[j]]<-
    ...
    stat_summary(fun.y=mean, geom="point", size=2, color="black", position=dodge) +
...
lastplot=annotate_figure(plot, top = text_grob("rd_9_violin", 
                                               color = "black", face = "bold", size = 14))
mywidth<-8
myheight<-4

ggsave(paste0('rd_9_violins.pdf'), plot = lastplot, device='pdf', scale=1, width=5*mywidth, height=5*myheight, unit=c("in"), dpi=200)
...
ggsave(paste0('rd_9_violins_opposite.pdf'), plot = lastplot, device='pdf', scale=1, width=5*mywidth, height=5*myheight, unit=c("in"), dpi=200)
...
write.xlsx2(table, "table_rd.xlsx", sheetName = "All", append=TRUE )
