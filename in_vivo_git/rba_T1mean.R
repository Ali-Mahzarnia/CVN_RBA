...
len=length(temp$T1_mean)
volumes=matrix(NA,length(file_list),333)
noreadcsf=c(148,152,161,314,318,327)



Atlas_path = '/Users/ali/Desktop/Aug23/CVN/rba/in_vivo/CHASSSYMM3AtlasLegends.xlsx'
...
for (i in 1:length(file_list)) {
  ...
  len=length(temp$T1_mean)
  # temp$T1_mean
  #print(sum(temp$T1_mean))
  # print( sum(temp$T1_mean[2:len]))
  #if (sum(temp$T1_mean[2:len])==0) {volumes[i,2:len] = 0}
  #else 
  ...
  temp[missings,]$T1_mean =0 
  temp[missings,]$structure = Atlas$Structure[Atlas$index%in%missings] 
  
  }
  
    volumes[i,2:333]=temp$T1_mean[1:332] #/ sum(temp$T1_mean[1:332])
  # print(sum(as.numeric(volumes[i,2:len])))
  # whole_volume[i,2]=sum(temp$T1_mean[-noreadcsf])
...
pathnames='/Users/ali/Desktop/Aug23/CVN/rba/in_vivo/mouse_anatomy.csv'
...
write.xlsx2(table, "table_T1mean.xlsx", sheetName = "Significant" )
...
lastplot=annotate_figure(plot, top = text_grob("T1mean_9_violin", 
                                               color = "black", face = "bold", size = 14))
mywidth<-8
myheight<-4

ggsave(paste0('T1mean_9_violins.pdf'), plot = lastplot, device='pdf', scale=1, width=5*mywidth, height=5*myheight, unit=c("in"), dpi=200)
...
ggsave(paste0('T1mean_9_violins_opposite.pdf'), plot = lastplot, device='pdf', scale=1, width=5*mywidth, height=5*myheight, unit=c("in"), dpi=200)
...
write.xlsx2(table, "table_T1mean.xlsx", sheetName = "All", append=TRUE )
