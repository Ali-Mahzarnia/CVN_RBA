...
scores = read_xlsx(filter_score_path) %>% select(`SAMBA Brunno` , score_CBF , treatment, cbf )
scores = na.omit(scores)


dodge <- position_dodge(width = 0.5 )
mycolors <- c('blueviolet', 'chartreuse1', 'red', 'azure3')
plot1 = ggplot(scores, aes(x=treatment, y=as.numeric(cbf), fill = treatment)) +
  ...
  ggrepel::geom_text_repel(aes(label =score_CBF ), box.padding = unit(2, 'lines'))+
  ...
  #panel_border() + 
  theme_bw()+
  labs(x = "Treatment", y = paste0("ROI CBF Mean"), title = paste0('Global CBF' ) ) +
...
print(aggregate(as.numeric(cbf)~treatment,data=scores,mean))
print(aggregate(as.numeric(cbf)~treatment,data=scores,sd))

mywidth<-8
myheight<-4

ggsave('CBF_global.pdf', plot = plot1, device='pdf', scale=1, width=mywidth, height=2*myheight, unit=c("in"), dpi=200)

lm <- lm( cbf ~  as.factor(treatment) ,data=scores )
an=anova(lm)
an
pvals=an$`Pr(>F)`
eff=effectsize::eta_squared(lm, partial = TRUE)

######
'''
scores_removal = scores[which(scores$score_CBF<4),]
